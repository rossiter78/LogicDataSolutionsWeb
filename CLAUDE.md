...
- **No em dashes.** Do not use the em dash character (`—`) anywhere in copy, metadata, alt/aria text, or documentation. Rephrase with a colon, a comma, parentheses, or two sentences instead. This applies to all future copy as well: nothing shipped or written for this project should contain `—`.
